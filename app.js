(() => {
  ...
  function setPage(n){page=Math.max(1,Math.min(TOTAL,n));img.src=`page-${String(page).padStart(2,"0")}.jpg`;img.alt=`Museum Posters and Models — page ${page} of ${TOTAL}`;indicator.textContent=`Page ${page} of ${TOTAL}`;$("prevBtn").disabled=page===1;$("nextBtn").disabled=page===TOTAL;location.hash=`page=${page}`}
